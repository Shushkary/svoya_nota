            ...
            <div className="nutrition-columns">
              <section className="n-panel">
                ...
                <div className="meal-methods">
                  ...
                  <button className="entry-action barcode-action" disabled={busy} title="Честный знак: сканировать DataMatrix или ввести GTIN" onClick={() => setBarcodeOpen(true)}><span className="entry-icon"><svg viewBox="0 0 24 24" width="21" height="21" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3.5" y="3.5" width="6.5" height="6.5" rx="1.2"/><rect x="14" y="3.5" width="6.5" height="6.5" rx="1.2"/><rect x="3.5" y="14" width="6.5" height="6.5" rx="1.2"/><path d="M14 14h3M17 14v3M14 17h3M20.5 14v3M14 20.5h3M17 17v3.5M20.5 17v3.5M17 20.5h3.5"/></svg></span><span className="entry-copy"><strong>Код</strong><small>Честный знак</small></span></button>
                </div>
                <button className="entry-manual" onClick={() => { setEditing(null); setFormNotice(''); setForm({ ...emptyForm(), time: formatHour(mealMinute / 60) }); }}>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 20h4l10.5-10.5a1.8 1.8 0 0 0-2.5-2.5L5.5 17.5 4 20z"/><path d="M13.5 6.5l3 3"/></svg>
                  Заполнить вручную, без ИИ
                </button>
                ...
                {notice && <p className="n-notice" role="status">{notice}</p>}
                <div className="n-control"><div><span>время приёма</span><b>{formatHour(mealMinute / 60)} · {mealType(mealMinute / 60)}</b></div><input type="range" min="0" max="1439" step="1" value={mealMinute} onChange={(event) => { setMealMinuteTouched(true); setMealMinute(Number(event.target.value)); }} /><small>сейчас — текущее время; после ручной настройки остаётся выбранное время</small></div>
